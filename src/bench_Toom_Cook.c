...
static void bench_k_for_size(int N, const int* ks, int nbk, int R) {
    ...
    for (int idx = 0; idx < nbk; idx++) {
        ...
        double* Cw = toom_cook_polynomial_multiplication(A, degA, B, degB, k, NAIVE_BASE);
        free(Cw);

        double t0 = now_seconds();
        for (int r = 0; r < R; r++) {
            double* C = toom_cook_polynomial_multiplication(A, degA, B, degB, k, NAIVE_BASE);
            free(C);
        }
        double t1 = now_seconds();
        double avg_ms = 1000.0 * (t1 - t0) / (double)R;

        double* Ccheck = toom_cook_polynomial_multiplication(A, degA, B, degB, k, NAIVE_BASE);
...
static void base_case(int k, int min_deg, int max_deg, int step_deg, int repetition) {
    printf("deg,k,time_naive_ms,time_karatsuba_ms,winner\n");
    ...
    for (int deg = min_deg; deg <= max_deg; deg += step_deg) {
        ...
        for (int base = NAIVE_BASE; base <= BASE_KARATSUBA; base++) {
            /* warm-up */
            double *Cw = toom_cook_polynomial_multiplication(
                A, deg, B, deg, k, (base_algo)base
            );
            free(Cw);

            double t0 = now_seconds();
            for (int r = 0; r < repetition; r++) {
                double *C = toom_cook_polynomial_multiplication(
                    A, deg, B, deg, k, (base_algo)base
                );
                free(C);
            }
        ...
        const char *winner =
            (average_time[NAIVE_BASE] <= average_time[BASE_KARATSUBA])
            ? "naive" : "karatsuba";

        printf(
            "%d,%d,%.6f,%.6f,%s\n",
            deg, k,
            average_time[NAIVE_BASE],
            average_time[BASE_KARATSUBA],
            winner
        );
